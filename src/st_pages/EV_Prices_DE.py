...
def main():
    ...
    mean_rule = alt.Chart(pd.DataFrame({'Price': [mean_price]})).mark_rule(color='red').encode(
    x='Price:Q')
    median_rule = alt.Chart(pd.DataFrame({'Price': [median_price]})).mark_rule(color='black').encode(
    x='Price:Q')
